		...
		dtype conv_zp, ftype conv_scale, dtype batch_zp, ftype batch_scale, int_t flag) // 0 for conv3d, 1 for conv3d_k
{
    ...
    for(int_t yi = 0; yi < (Y_num[1]/YC); yi++){
        ...
        for(int_t xi = 0; xi < (X_num[1]/XC); xi++){
            ...
            if(flag == 0)
				Conv3d(X_bram, X_num, xi, XC, Y_bram, Y_num, yi, YC, Kernel_bram, Kernel_num, stride, padding, 56);
            else if(flag == 1){
				int_t i = 0;
        ...
        for(int_t c = 0; c < YC; c++){
            ...
            for(int_t k = 0; k < Y_num[2]*Y_num[3]*Y_num[4]; k++){
                int_t tmp = (int_t)roundf((((Y_bram[offset+k]*3.756307810544967651e-02*Kernel_stem_0_scale[yi*YC+c] - Mu_stem_1[yi*YC+c]) / sqrtf(Var_stem_1[yi*YC+c]+0.00001)) * Gamma_stem_1[yi*YC+c] + Bias_stem_1[yi*YC+c])/0.07323520630598068237);
