							...
							for(int kd = 0; kd < KD; kd++)
								for(int kh = 0; kh < KH; kh++)
									for(int kw = 0; kw < KW; kw++)
										X_out.data[n*C_out*D_out*H_out*W_out + c*D_out*H_out*W_out + d*H_out*W_out + h*W_out + w] += X_data[n][c][d*stride[0]+kd][h*stride[1]+kh][w*stride[2]+kw] * Kernel_data[c][kc][kd][kh][kw];
	return;
}
