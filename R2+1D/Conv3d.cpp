							...
							for(int_t kh = 0; kh < Kernel_num[1]; kh++){
								...
								for(int_t kw = 0; kw < Kernel_num[2]; kw++){
									...
									if(dPos >= 0 && hPos >= 0 && wPos >= 0 && dPos < X_num[2] && hPos < X_num[3] && wPos < X_num[4])
										tmp_Y += ((int_t)X_data[xc*X_num[2]*X_num[3]*X_num[4] + dPos*X_num[3]*X_num[4] + hPos*X_num[4] + wPos]- zp_in) * Kernel_data[(yi*YC + yc)*X_num[1]*Kernel_num[0]*Kernel_num[1]*Kernel_num[2] + (xi*XC + xc)*Kernel_num[0]*Kernel_num[1]*Kernel_num[2] + kd*Kernel_num[1]*Kernel_num[2] + kh*Kernel_num[2] + kw];
								}
							}
						}
					}
