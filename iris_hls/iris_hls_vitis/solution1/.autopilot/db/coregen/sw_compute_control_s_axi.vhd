...
entity sw_compute_control_s_axi is
generic (
...
end entity sw_compute_control_s_axi;
...
architecture behave of sw_compute_control_s_axi is
    ...
    constant ADDR_IM_DATA_0    : INTEGER := 16#10#;
    constant ADDR_IM_DATA_1    : INTEGER := 16#14#;
    constant ADDR_IM_CTRL      : INTEGER := 16#18#;
    ...
    signal int_im              : UNSIGNED(63 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_IM_DATA_0 =>
                        rdata_data <= RESIZE(int_im(31 downto 0), 32);
                    when ADDR_IM_DATA_1 =>
                        rdata_data <= RESIZE(int_im(63 downto 32), 32);
...
-- ----------------------- Register logic ----------------
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_IM_DATA_0) then
                    int_im(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_im(31 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_IM_DATA_1) then
                    int_im(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_im(63 downto 32));
                end if;
            end if;
        end if;
    end process;
