...
typedef struct {
    ...
    XIris_module_uio_map maps[ MAX_UIO_MAPS ];
} XIris_module_uio_info;

/***************** Variable Definitions **************************************/
static XIris_module_uio_info uio_info;
...
static int uio_info_read_name(XIris_module_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XIris_module_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XIris_module_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XIris_module_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XIris_module_Initialize(XIris_module *InstancePtr, const char* InstanceName) {
	XIris_module_uio_info *InfoPtr = &uio_info;
