...
typedef struct {
    ...
    XSw_compute_uio_map maps[ MAX_UIO_MAPS ];
} XSw_compute_uio_info;

/***************** Variable Definitions **************************************/
static XSw_compute_uio_info uio_info;
...
static int uio_info_read_name(XSw_compute_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XSw_compute_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XSw_compute_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XSw_compute_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XSw_compute_Initialize(XSw_compute *InstancePtr, const char* InstanceName) {
	XSw_compute_uio_info *InfoPtr = &uio_info;
